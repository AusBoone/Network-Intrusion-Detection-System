...
struct SourceEntry *sources = NULL; // Linked list head for source entries
...
void AnalyzeTcpAttack(const struct pcap_pkthdr *header, const u_char *packet) {
  ...
  if (tcp_header->syn && !tcp_header->ack) {
    struct SourceEntry *current = sources;
...
void AnalyzeUdpAttack(const struct pcap_pkthdr *header, const u_char *packet) {
  // Analyze UDP packets here

  // Example: UDP Flood attack detection
  // More logic needed to determine UDP Flood
